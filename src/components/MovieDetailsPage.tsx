...
import { Play, Film, Star, Plus, ChevronLeft, Heart, Bookmark } from 'lucide-react';
...
export default function MovieDetailsPage() {
    ...
    const loadMovie = async () => {
      ...
      try {
        ...
        if (!data.videos?.results || data.videos.results.length === 0) {
          try {
            const enData = await tmdbService.getMovieDetails(id, 'en-US');
  ...
  return (
    ...
    >
      <AnimatePresence>
        {showPlayer && trailer && movie && (
          <VideoPlayer 
            key={`movie-trailer-${movie.id}`}
